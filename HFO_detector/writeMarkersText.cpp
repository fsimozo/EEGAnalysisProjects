...
void writeMarkersText::writeMethodResults(methodResult* result, QDir path, QString ch_label, QString method_name, int channel_index){
    ...
    file_names[14]="15-P8-O2";
    file_names[15]="16-FZ-CZ";
    file_names[16]="17-CZ-PZ";
    file_names[17]="18-P7-T7";
    file_names[18]="19-T7-FT9";
    file_names[19]="20-FT9-FT10";
    file_names[20]="21-FT10-T8";
    
    QString output;
    //Save point information
//    for(int i=0; i<50; i++){
//        if(!(QFile(path.absolutePath()+"/"+"DFA analysis"+"/points"+"_"+QString::number(i)+".txt").exists())){
////                printf((path.absolutePath()+"/"+"DFA analysis"+"/graphs"+"_"+QString::number(i)+".txt").toAscii()+"\n");
//                ch_label = QString::number(i);
//                break;
//        }
//    }
...
//        output.replace(".", ",", Qt::CaseSensitive);
    ...
    if((output=result->getGraphs()).count()>0){
        QFile graph_file(path.absolutePath()+"/graphs"+"_"+ch_label+".txt");
